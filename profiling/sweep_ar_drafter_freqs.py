...
parser.add_argument("--veri_freqs", type=int, nargs="+",  # one or more int frequencies
                    default=[5],
                    help="Frequency of verification steps (in number of tokens) (e.g., --veri_freqs 3 5 7)")
...
# args.drafter_configs = [("ar", None, 5)] if args.run_ar else []  # draft_type, drafter_threshold, veri_freq
args.drafter_configs = [("ar", None, thr) for thr in args.veri_freqs] if args.run_ar else []  # draft_type, drafter_threshold, veri_freq
...
# for problem_id in [12]:
    ...
    for draft_type, drafter_threshold, veri_freq in args.drafter_configs:
        ...
        if args.read_pickle:
            ...
            logging.info(f"{Colors.BOLD}=== [Problem {problem_id}] Running drafter: {draft_type}_{drafter_threshold}_{veri_freq} ==={Colors.RESET}")
            accepted_tokens = 0
            rejected_tokens = 0
            ...
            while len(current_token_ids) < len(target_ids):
                logging.debug(f"--- [{draft_type}_{drafter_threshold}_{veri_freq}] Speculation round {num_speculation_rounds} ---")
